...
paneller=function(row = 1,column=1)
{
  ...
  if(row == 1 & column == 2){
    ...
    plotIncSim = data.table(TIME = outputTotal$time,
                            MEDIAN = apply(outputTotal$IncNotifiedLinkedSim,2, median, na.rm = T),
    ...
    grid.lines(t,plotIncSim$MEDIAN,default.units = 'native',gp=gpar(col=col2a)) 
  ...
  if(row == 2 & column == 1){
    ...
    plotIncSim = data.table(TIME = outputTotal$time,
                            MEDIAN = apply(outputTotal$IncNotifiedUnlinkedSim,2, median, na.rm = T),
  ...
  if(row == 2 & column == 2){
    ...
    plotIncSim = data.table(TIME = outputTotal$time,
                            MEDIAN = apply(outputTotal$IncMissedSim_DOI,2, median, na.rm = T),
    ...
    grid.polygon(c(t,rev(t)),
                 ...
                 default.units = 'native',gp=gpar(col=NA,fill=col2b))
    grid.lines(t,plotIncSim$MEDIAN,default.units = 'native',gp=gpar(col=col1))
